...
public class MergeTwo2DArrays {
    ...
    /**
     * Merges two 2D arrays by combining rows with the same key and summing their values using a map.
     * If a key exists in only one of the arrays, its corresponding row will be included as is.
     *
     * @param nums1 the first 2D array where each row contains a key-value pair
     * @param nums2 the second 2D array where each row contains a key-value pair
     * @return a new 2D array containing merged rows sorted by key, with summed values for matching keys
     */
    public int[][] mergeArraysUsingMap(int[][] nums1, int[][] nums2) {
